...
def augment_seq(seq, fcgr_instance, augmentation_type="None", mutation_rate=None, frag_perc=None):
    ...
    if augmentation_type == "mutation":
        augmented_seq = mutation_optimized(seq,
                                         transition=True,
                                         transversion=True,
                                         transition_prob=mutation_rate,
                                         transversion_prob=(0.5 * mutation_rate))
        fcgr_repr = fcgr_instance(augmented_seq)
        
    elif augmentation_type == "fragmentation":
        frag_len = int(frag_perc * len(seq))
        augmented_seq = fragmentation(seq, frag_len) 
        fcgr_repr = fcgr_instance(augmented_seq)
        
    else:
        raise ValueError("Unsupported augmentation type specified.")

    return fcgr_repr, augmented_seq
...
def generate_pairs(data: pd.DataFrame,
    ...
    X_train = np.zeros((total_pairs, 2), dtype="object")
    
  
    X_train_seqs = np.zeros((total_pairs, 2), dtype="object")
    ...
    for idx, record in tqdm(data.iterrows(), total=data.shape[0]):
        ...
        for _ in range(number_of_pairs):
            ...
            if mutation_rate_weak is not None:
                ...
                weak_fcgr, weak_text = augment_seq(
                    seq=original_seq,
                    fcgr_instance=fcgr,
                    augmentation_type="mutation",
                    mutation_rate=mutation_rate_weak
                )
                strong_fcgr, strong_text = augment_seq(
                    seq=original_seq,
                    fcgr_instance=fcgr,
                    augmentation_type="mutation",
            ...
            X_train_seqs[pair_idx] = (weak_text, strong_text)
    ...
    print(f"Shape of X_train_seqs: {X_train_seqs.shape}") 
    print(f"Shape of X_test: {X_test.shape} | Number of labels in y_test: {len(y_test)}")

    return X_train, X_test, y_test, X_train_seqs
